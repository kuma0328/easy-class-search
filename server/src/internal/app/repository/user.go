...
func (ur *userRepository) GetActiveTabById(ctx context.Context, id string) (domain.ActiveTab, error) {
	query := `
	...
	if err != nil {
		return domain.ActiveTab{}, nil
	}
	return activeTab, nil
}

func (ur *userRepository) AddActiveTabById(ctx context.Context, activeTab domain.ActiveTab) error {
	query := `
	INSERT INTO active_tab (id, active_tab)
	VALUES ($1, $2)
	ON CONFLICT (id)
	DO UPDATE SET
